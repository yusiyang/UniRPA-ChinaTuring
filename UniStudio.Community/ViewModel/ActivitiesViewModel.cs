...
namespace UniStudio.Community.ViewModel
{
    ...
    public class ActivitiesViewModel : ViewModelBase
    {
        ...
        private string AvailableActivitiesXmlPath
        {
            get
            {
                ...
                if (!File.Exists(_availableActivitiesXmlPath))
                {
                    ...
                    using (var ms = new MemoryStream(UniStudio.Community.Properties.Resources.AvailableActivities))
                    {
                        ms.Flush();
                        ms.Position = 0;
                        doc.Load(ms);
        ...
        public string SearchText
        {
            ...
            set { _searchTextProperty = value; RaisePropertyChanged(() => SearchText); }
        }
        ...
        private void InitNameMap()
        {
            ...
            using (var ms = new MemoryStream(UniStudio.Community.Properties.Resources.ActivitiesEnglishMap))
            {
                ms.Flush();
                ms.Position = 0;
                doc.Load(ms);
