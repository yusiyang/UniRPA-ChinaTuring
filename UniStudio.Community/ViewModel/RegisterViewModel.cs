...
using Common = UniStudio.Community.Librarys.Common;
...
namespace UniStudio.Community.ViewModel
{
    ...
    public class RegisterViewModel : ViewModelBase
    {
                    ...
                    () =>
                    {
                        ...
                        if (ret == true)
                        {
                            ...
                            try
                            {
                                ...
                                var result = UniMessageBox.Show(m_view, "导出机器码成功，是否定位到该机器码文件？", "提示", MessageBoxButton.YesNo, MessageBoxImage.Question, MessageBoxResult.Yes);
                            ...
                            {
                                UniMessageBox.Show(m_view, "导出机器码成功失败，请检查", "错误", MessageBoxButton.OK, MessageBoxImage.Warning);
                            }
        ...
        private void processImportAuthorizationFile(string fileFullPath)
        {
            ...
            try
            {
                if (IsNotExpired(fileFullPath, ref expiresDate))
                {
                    ...
                    UniMessageBox.Show(m_view, "授权成功，请重启程序！", "提示", MessageBoxButton.OK, MessageBoxImage.Information);
                    Environment.Exit(0);
                }
                else
                {
        ...
        {
            public MyComputerInfo MachineCodeInfo { get; set; }
                       ...
                       ?? (_activateCommand = new RelayCommand(
                           async () =>
                           {
                               using (var loadingWait = LoadingWait.Show())
                               {
                                   ...
                                   {
                                       UniMessageBox.Show(m_view, "请输入激活码！", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                                       return;
                                   }
                                   var url = ConfigurationManager.AppSettings["ActiveApi"];
                                   var activation = new Activation
                                   {
                                       MachineCodeInfo = MyComputerInfo.Instance(),
                                       ActivationCode = Code
                                   };

                                   var content = JsonConvert.SerializeObject(activation);
                                   content = RSACommon.EncryptString(content, Properties.Resources.verify_public_rsa);//加密

                                   var requestResult = await HttpRequest.PostAsync(url, $"\"{content}\"", "application/json");

                                   if (string.IsNullOrEmpty(requestResult))
                                   {
                                       UniMessageBox.Show(m_view, "尝试激活出错，请重试！", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                                       return;
                                   }

                                   var result =
                                       Newtonsoft.Json.JsonConvert.DeserializeObject<ActivationResult>(requestResult);
                                   ...
                                   if (result.Code == 200)
                                   {
                                       ...
                                       File.WriteAllText(targetPath, result.Data.ToString());
                                   ...
                                   UniMessageBox.Show(m_view, result.Message, "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                               }
                           }));
            }
        }
