...
namespace UniWorkforce.ViewModel
{
    ...
    public class UserPreferencesViewModel : ViewModelBase
    {
        ...
        public void LoadSettings()
        {
            ...
            var path = Settings.Instance.LocalRPAStudioDir + @"\Config\UniWorkforce.settings";
            doc.Load(path);
        ...
        public RelayCommand ResetSettingsCommand
        {
            get
            {
                return _resetSettingsCommand
                    ...
                    {
                        //重置默认设置
                        var ret = UniMessageBox.Show(m_view, "确认重置为默认设置吗？", "询问", MessageBoxButton.OKCancel, MessageBoxImage.Question, MessageBoxResult.Cancel);
