                        ...
                        using (var loadingWait = LoadingWait.Show())
                        {
                            try
                            {
                                if (ViewModelLocator.instance.Register.IsNotExpired())
                                {
                                    ...
                                    {
                                        UniMessageBox.Show(App.Current.MainWindow, "请填写唯一码！", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
                                        return;
                                    }
                                    ...
                                    {
                                        UniMessageBox.Show(App.Current.MainWindow, "未授权", "连接失败", MessageBoxButton.OK, MessageBoxImage.Error);
                                        return;
                                    }
                                ...
                                {
                                    UniMessageBox.Show(App.Current.MainWindow, "请先注册产品后再连接", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                                }
                            }
                            catch(Exception ex)
                            {
