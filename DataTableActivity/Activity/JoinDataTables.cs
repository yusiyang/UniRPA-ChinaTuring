...
namespace DataTableActivity
{
    ...
    public sealed class JoinDataTables : AsyncCodeActivity
    {
        ...
        [RequiredArgument]
        [DisplayName("数据表 1")]
        [Description("要在此操作中使用的第一个表，存储在 DataTable 变量中。该字段仅支持 DataTable 变量.")]
        public InArgument<DataTable> DataTable1 { get; set; }

        [Category("输入")]
        [RequiredArgument]
        [DisplayName("数据表 2")]
        [Description("要在此操作中使用的第二个表，存储在 DataTable 变量中。该字段仅支持 DataTable 变量。")]
        ...
        [Category("选项")]
        [DisplayName("连接类型")]
        [Description("数据表连接类型。可有的选项包括：Inner、Left、Full。")]
        ...
        [Browsable(false)]
        public List<JoinOperationArgument> Arguments { get; set; }
        ...
        public string icoPath
        {
            get
            {
                return @"pack://application:,,,/Plugins.Shared.Library;Component/Resource/Image/Activities/DataTable/join-data-table.png";
            }
        }
        ...
        private bool ApplyJoinArgumentsOnRows(DataRow leftRow, DataRow rightRow, List<SimplifiedArgument> joinArguments)
        {
            foreach (SimplifiedArgument argument in joinArguments)
            {
                ...
                {
                    //Trace.TraceWarning(string.Format(UiPath_System_Activities.JoinWarning, new object[] { argument.Column1, leftRow, argument.Column2, rightRow }));
                    return false;
                }
        ...
        private DataTable JoinTables(DataTable dataTable1, DataTable dataTable2, ActivityContext context)
        {
            ...
            {
                //throw new Exception(UiPath_System_Activities.NoJoinArguments);
            }
