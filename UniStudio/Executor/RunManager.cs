    ...
    public class RunManager
    {
        ...
        public void Stop()
        {
            if (m_app != null)
            {
                try
                {
                    var asyncResult= m_app.BeginTerminate("执行已取消", null, null);
                    var hwndSource = PresentationSource.FromVisual(Application.Current.MainWindow) as HwndSource;

                    //关闭非主窗口以外的其他窗口
                    WindowsHelper.CloseWindowsExcludeWindow(hwndSource.Handle);
                    m_app.EndTerminate(asyncResult);
                }
                catch (Exception ex)
                {
