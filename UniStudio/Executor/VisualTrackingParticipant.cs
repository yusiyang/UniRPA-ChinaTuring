...
namespace UniStudio.Executor
{
    public class VisualTrackingParticipant : TrackingParticipant
    {
        ...
        {
            if (m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.Stop)
            {
        ...
        protected void OnTrackingRecordReceived(TrackingRecord record, TimeSpan timeout)
        {
            ...
            else if (record is ActivityScheduledRecord)
            {
                ...
                if (activityScheduledRecord.Child != null && m_activityIdToWorkflowElementMap.ContainsKey(activityScheduledRecord.Child.Id))
                {
                    ...
                    {
                        if (m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.Null
                        || m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.Continue
                        )
                        {
                            processSlowStep(activityScheduledRecord.Child.Id);
                        }
                        else if (m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.Break)
                        {
                            processWait(activityScheduledRecord.Child.Id);
                            m_lastDebugActivityScheduledRecord = activityScheduledRecord;
                        }
                        else if (m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.StepInto)
                        {
                            processWait(activityScheduledRecord.Child.Id);
                            m_lastDebugActivityScheduledRecord = activityScheduledRecord;
                        }
                        else if (m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.StepOver)
                        {
            ...
            else if (record is ActivityStateRecord)
            {
                ...
                if (activityStateRecord.State == ActivityStates.Closed
                    ...
                    && (m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.Null
                        || m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.Continue
                        )
                    )
                {
                ...
                if (activityStateRecord.State == ActivityStates.Closed
                    ...
                    && (
                    m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.StepInto
                    || m_debuggerManager.NextOperate == DebuggerManager.DebugOperate.StepOver
                         )
                    )
                {
