...
namespace UniStudio.ViewModel
{
    ...
    public class SplashScreenViewModel : ViewModelBase
    {
        ...
        private bool UpgradeSettings()
        {
            ...
            using (var ms = new MemoryStream(UniStudio.Properties.Resources.UniStudio_settings))
            {
                ms.Flush();
                ms.Position = 0;
                docNew.Load(ms);
        ...
        public bool IsNotExpired(string fileFullPath, ref string expiresDate)
        {
            ...
            if (Plugins.Shared.Library.Librarys.Common.CheckAuthorization(fileFullPath, UniStudio.Properties.Resources.verify_public_rsa, ref expiresDate))
            {
                string authString = System.IO.File.ReadAllText(fileFullPath);
                ...
                //企业版不能使用社区版授权
                if (!(bool)registerObj["IsCommunity"])
                {
                    //授权合法，检查下有效期
