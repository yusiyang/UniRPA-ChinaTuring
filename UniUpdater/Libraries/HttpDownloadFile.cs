...
namespace UniUpdater.Libraries
{
    class HttpDownloadFile
    {
        ...
        /// <returns></returns>
        public bool Download(string url, string localfile,string md5)
        {
            Url = url;
            var originFileName = Common.GetFileNameFromUrl(Url);
            ...
            {
                var fileMd5 = Common.GetMD5HashFromFile(finishedFilePath);
                if (md5.ToLower() == fileMd5.ToLower())
                {
