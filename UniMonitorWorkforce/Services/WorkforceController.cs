using log4net;
using Plugins.Shared.Library.Librarys;
using System;
using System.IO;
using UniWorkforce.Librarys;
using UniWorkforce.ViewModel;

namespace UniWorkforce.Services
{
    public class WorkforceController
    {
        private static readonly ILog logger = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        /// <summary>
        /// 发布流程
        /// </summary>
        /// <param name="packageModel"></param>
        public void PublishProcess(PackageModel packageModel)
        {
            var tempZipDir = Path.Combine(Settings.Instance.LocalRPAStudioDir, "tempFiles");
            if (!Directory.Exists(tempZipDir))
            {
                Directory.CreateDirectory(tempZipDir);
            }

            var filePath = Path.Combine(tempZipDir, $"{packageModel.PackageName}.zip");
            if(File.Exists(filePath))
            {
                File.Delete(filePath);
            }
            using (var fileStream = new FileStream(filePath, FileMode.OpenOrCreate))
            {
                fileStream.Write(packageModel.File.ToByteArray(), 0, packageModel.File.Length);
            }

            var unZipDir=filePath.Replace(Path.GetFileName(filePath), Path.GetFileNameWithoutExtension(filePath));
            if (!unZipDir.EndsWith("/"))
            {
                unZipDir += "/";
            }
            try
            {
                ZipHelper.UnZip(filePath, unZipDir);
            }
            catch(Exception ex)
            {
                Logger.Error(ex, logger);
                return;
            }

            foreach (var file in Directory.GetFileSystemEntries(unZipDir))
            {
                var monitorProcessFile = Path.Combine(tempZipDir, packageModel.PackageName, $"{packageModel.PackageName}.{packageModel.Version}.nupkg");
                if(file.Replace('/','\\')== monitorProcessFile)
                {
                    var monitorTargetProcess = Path.Combine(Settings.Instance.PackagesDir, $"{packageModel.PackageName}.nupkg");
                    File.Copy(monitorProcessFile, monitorTargetProcess, true);
                }
                else
                {
                    var fileName = Path.GetFileName(file);
                    var monitorTarget = Path.Combine(Settings.Instance.MonitorTargetDir, fileName);

                    File.Copy(file, monitorTarget, true);
                }
            }

            DirectoryOperation.DeleteDir(Settings.Instance.InstalledPackagesDir);

            ViewModelLocator.instance.Main.RefreshCommand.Execute(null);
            var packageItems = ViewModelLocator.instance.Main.PackageItems;
            if (packageItems?.Count > 0)
            {
                var packageCount = packageItems.Count;
                for (var i = 0; i < packageCount; i++)
                {
                    packageItems[i].UpdateCommand.Execute(null);
                }
            }
        }

        /// <summary>
        /// 根据名称获取PackageItem
        /// </summary>
        /// <param name="name"></param>
        /// <returns></returns>
        private static PackageItem GetPackageItemByName(string name)
        {
            var packageItems = ViewModelLocator.instance.Main.PackageItems;
            foreach (var item in packageItems)
            {
                if (item.Name == name)
                {
                    return item;
                }
            }
            return null;
        }

        /// <summary>
        /// 运行流程
        /// </summary>
        /// <param name="packageName"></param>
        public void RunProcess(string packageName)
        {
            var packageItem = GetPackageItemByName(packageName);
            if (packageItem != null)
            {

                packageItem.StartCommand.Execute(null);
            }
        }

        /// <summary>
        /// 停止运行
        /// </summary>
        public void StopProcess()
        {
            ViewModelLocator.instance.Main.StopCommand.Execute(null);
        }
    }
}
