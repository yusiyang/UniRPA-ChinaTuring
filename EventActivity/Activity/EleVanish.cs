...
namespace EventActivity
{
    ...
    public sealed class EleVanish : NativeActivity
    {
        #region 属性分类：常见
        ...
        [Category("输入")]
        [DisplayName("等待未激活")]
        [Description("选中该复选框时，该活动将在执行任何其他操作之前等待指定用户界面元素不再处于活动状态。")]
        public bool WaitNotActivity { get; set; }

        [Category("输入")]
        [DisplayName("等待不可见")]
        [Description("选中该复选框时，该活动将在执行任何其他操作之前等待指定用户界面元素在屏幕上不再可见。")]
        ...
        public string icoPath
        {
            get
            {
                return @"pack://application:,,,/Plugins.Shared.Library;Component/Resource/Image/Activities/Event/ele-vanish.png";
            }
        }
        ...
        [Browsable(false)]
        public string DefaultName { get { return "捕获元素消失"; } }
                    ...
                    Task waitTask = new Task(() =>
                    {
                        ...
                        while (flag)  //不可见时跳出
                        {
                            lock (this)
                            {
                                ...
                                if (WaitNotVisible && !element.IsVisible())
                                {
                                    flag = false;
                                    continue;
                                }

                                if (WaitNotActivity && !UiCommon.IsForeground(element))
                                {
                                    flag = false;
                                    continue;
                                }
