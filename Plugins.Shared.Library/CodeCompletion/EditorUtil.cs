...
namespace Plugins.Shared.Library.CodeCompletion
{
    ...
    {
        public static ExpressionNode autoCompletionTree = new ExpressionNode();

        /// <summary>
        /// 主程序UniStudio引用的所有程序集
        /// </summary>
        public static List<Assembly> MainReferenceAssemblys { get; set; }

        public static void init(List<Assembly> references = null)
        {
            if(references != null)
            {
                MainReferenceAssemblys = references;
            }
            
            autoCompletionTree = CreateDefaultAutoCompletionTree(MainReferenceAssemblys);
        }
        /// <summary>Builds a default auto-completion navigation tree based on the currently loaded 
        /// assemblies.</summary>
        public static ExpressionNode CreateDefaultAutoCompletionTree(List<Assembly> references)
        {
...
(childType.IsPublic && childType.IsVisible && childType.Namespace != null)
        ...
        public static void AddTypeToExpressionTree(ExpressionNode target, Type child)
        {
            ...
            //代码提示初始化限制只获取类名,并且排除掉泛型类
            if (child.IsClass && !child.IsGenericType)
            {
