...
namespace Plugins.Shared.Library.Editors
{
    ...
    public partial class VBNetCodeEditorDialog
    {
        ...
        private List<VariableNameType> variableDeclarations = new List<VariableNameType>();
        private IEnumerable<string> _imports = null;
        ...
        public VBNetCodeEditorDialog()
        {
            InitializeComponent();
            textEditor.DocumentTextChanged += TextEditor_DocumentTextChanged;
            textEditor.GotFocus += TextArea_GotFocus;
        }

        public void Init(ModelItem ownerActivity) {
            ...
            textEditor.InitVBLanguage(_imports);

            //设置代码前后文本（用户只需要编辑显示中间部分就好了）
            textEditor.Document.SetHeaderAndFooterText(m_codeHeader, m_codeFooter);
            #endregion
            ...
            foreach (var arg in args)
            {
                variableDeclarations.Add(new VariableNameType
                {
                    ...
                    VariableType = arg.Value.ArgumentType
                });
            }
        }
        ...
        private void TextEditor_DocumentTextChanged(object sender, EditorSnapshotChangedEventArgs e)
        {
            ...
            //        if (e.TypedText == " ")
            //        {
            //            ShowCompletionSession(string.Empty);
            //        }
            //        else
            //        {
            //            var txt = e.NewSnapshot.Text?.Trim();
            //            ShowCompletionSession(txt);
            //        }
            //    }
            //}
        }

        //void ShowCompletionSession(string exprFullNameString)
        //{
        //    try
        //    {
        //        var session = GlobalEditorConfig.GetCompletionSession(exprFullNameString, variableDeclarations,m_namespaceNodeRoot);
        ...
        public string GetCodeHeader(Dictionary<string, Argument> args)
        {
            ...
            _imports = EnumerateImports(base.Context, base.ModelItem);
            string importsHeader = GetImports(_imports);
            string vbNetArguments = GetVbNetArguments(inArgs);
            return $"{importsHeader}\r\nModule RPACodeRunner\r\nSub Run({vbNetArguments})\r\n";
        }
