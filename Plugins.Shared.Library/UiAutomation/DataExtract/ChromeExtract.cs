...
{
    public class ChromeExtract : IDataExtract
    {
        public string GetColumnData(UiElement element, string extractOption)
        {
            //return NativeMessageManager.GetColumnData((element?.uiNode as HtmlUiNode)?.CustomId, extractOption);
            var node = element?.uiNode as HtmlUiNode;
            if (node == null)
            {
                return "";
            }
...
";
            return node._browser.InjectAndRunJs(jsCode, extractOption, node);
        }

        public void ColorColumn(UiElement element, string name)
        {
            //NativeMessageManager.ColorColumn((element?.uiNode as HtmlUiNode)?.CustomId, name);
            var node = element?.uiNode as HtmlUiNode;
            if (node == null)
            {
                return;
    ...
    for (var i = 0; i < all.length; ++i) {
        ...
        if (e.getAttribute(""data-extract-result-name"", 0) === colName) {
            ...
            node._browser.InjectAndRunJs(jsCode, "", node);
        }

        public bool Compare(UiElement element1, UiElement element2)
        {
            return element1?.uiNode is HtmlUiNode node1 && element2?.uiNode is HtmlUiNode node2 &&
                   node1.CustomId == node2.CustomId;
        }
    ...
    for (var i = 0; i < all.length; ++i) {
        ...
        {
            //var ids = (element?.uiNode as HtmlUiNode)?.CustomId.Split('|');
            //if (ids?.Length < 3)
            //{
            //    return;
            //}

            //var tabId = int.Parse(ids[0],CultureInfo.CurrentCulture);
            //var customId = ids[1] + '|' + ids[2];
            //NativeMessageManager.SetHtmlAttribute(tabId, customId, attrName, attrValue);
            var node = element?.uiNode as HtmlUiNode;
        ...
        {
            //return NativeMessageManager.GetSameColumn((element?.uiNode as HtmlUiNode)?.CustomId, input);
            var node = element?.uiNode as HtmlUiNode;
            if (node == null)
            {
                return "";
    ...
    return outputXml;
            ...
            return node._browser.InjectAndRunJs(jsCode, input, node);
        }
