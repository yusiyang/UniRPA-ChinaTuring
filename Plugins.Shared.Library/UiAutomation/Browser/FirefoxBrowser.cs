...
namespace Plugins.Shared.Library.UiAutomation.Browser
{
    ...
    {
        private static readonly MessageClient Client = new MessageClient("NativeHost-Firefox");

        public static int TabCount => GetTabCount();

        public FirefoxBrowser(UiNode node)
        {
            if (!Equals(node?.WindowHandle, Client.BrowserHandle))
            {
                Available = false;
                return;
            }
        ...
        public FirefoxBrowser()
        {
            ...
            TabId = GetActiveTabId();
            UiNode = new UIAUiNode(UIAUiNode.UIAAutomation.FromHandle(Client.BrowserHandle));
            Available = true;
        }

        public FirefoxBrowser(string title)
        {
            var tabId = GetHtmlElemById(-1, "firefox.exe", title, null, null);
        ...
        public string Name => BrowserType.Firefox.ToString();
        public UiNode UiNode { get; private set; }
        public IntPtr Hwd => UiNode.WindowHandle;
        ...
        {
            var oldTabId = TabId;
            Process.Start("firefox.exe", $"{arguments} {uri}");
