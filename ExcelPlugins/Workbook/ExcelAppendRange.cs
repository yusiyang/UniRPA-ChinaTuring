...
namespace ExcelPlugins
{
    ...
    [Designer(typeof(ExcelAppendRangeDesigner))]
    public sealed class ExcelAppendRange : AsyncCodeActivity
    {
        #region 属性分类：常见
        ...
        InArgument<string> _ColumnBegin;
        [Category("输入")]
        [RequiredArgument]
        [DisplayName("起始列")]
        [Description("例：\"A\"。必须将文本放入引号中。")]
        public InArgument<string> ColumnBegin
        {
            get
            {
                return _ColumnBegin;
        ...
        [Browsable(false)]
        public string ClassName { get { return "ExcelAppendRange"; } }

        #endregion


        public ExcelAppendRange()
        {
            SheetName = "Sheet1";
            ColumnBegin = "A";
        }
        ...
        protected override IAsyncResult BeginExecute(AsyncCodeActivityContext context, AsyncCallback callback, object state)
        {
            ...
            try
            {
                ...
                string columnBegin = ColumnBegin.Get(context);
                System.Data.DataTable dt = DataTable.Get(context);
                string cellBegin = columnBegin + "1";
                ...
                int iRowBegin = rowEnd + 1;
                int iColBegin = sheet.Range[cellBegin, cellBegin].Column;

                StartWriteRange(dt, sheet, columnBegin+iRowBegin.ToString(), HasTitle);
