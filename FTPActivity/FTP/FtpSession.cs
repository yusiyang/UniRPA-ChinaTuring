        ...
        async Task IFtpSession.DeleteAsync(string path, CancellationToken cancellationToken)
        {
            ...
            switch (await ((IFtpSession)this).GetObjectTypeAsync(path, cancellationToken))
            {
                ...
                case FtpObjectType.File:
                    ...
                    break;
                default:
                    throw new NotImplementedException("FtpObjectType Type Not Support");
            }
        }
        ...
        async Task IFtpSession.UploadAsync(string localPath, string remotePath, bool overwrite, bool recursive, CancellationToken cancellationToken)
        {
            ...
            if (Directory.Exists(localPath))
            {
                ...
                foreach (Tuple<string, string> pair in listing)
                {
                    ...
                    await _ftpClient.UploadFileAsync(pair.Item1, pair.Item2, overwrite ? FtpExists.Overwrite : FtpExists.Skip, true, FtpVerify.None, null, cancellationToken);
                }
            }
            else
            {
                if (File.Exists(localPath))
                {
                    ...
                    await _ftpClient.UploadFileAsync(localPath, remotePath, overwrite ? FtpExists.Overwrite : FtpExists.Skip, true, FtpVerify.None, null, cancellationToken); 
                }
                else
                {
                    throw new ArgumentException("PathNotFoundException", (localPath));
